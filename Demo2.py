...
if 'triple.pkl' not in os.listdir():
    #table 데이터
    table_dir = "tables"
    ...
    with open('triple.pkl', 'wb') as f:
        pickle.dump(spo_set,f)


#asyncio를 활용한 query 검색

with open('triple.pkl', 'rb') as f:
...
async def main():
    ...
    def result_return(spo):
        ...
        query2_2 = "\"".join(["""SELECT ?subject ?property ?object 
                            WHERE 
                            { 
                            ?subject rdfs:label """, str(s), """@en.
                            ?subject ?property ?object.
        ...
        query3_1 = "\"".join(["""SELECT distinct ?subject ?property ?object 
                            ...
                            """])
        query3_2 = "\"".join(["""SELECT distinct ?subject ?property WHERE { ?subject rdfs:label ?s. {?subject ?property """,
                    str(o) ,
                    """. } UNION {?subject ?property """,
                    str(o),
                    """@en.} UNION {?subject ?property """,
                    str(o),
                    """^^xsd:dateTime.} FILTER (regex(str(?s) , """,
                    str(s),
                    """)) SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. } }"""])
    ...
    async def property_query(semaphore, spo):
        async with semaphore:
            ...
            #상태표시용
            print("current iter : {0}    running_num : {1}        result : {2}".format(num, running_num, result), end="\r", flush=True)
            '''if num % 100 == 0:
                print(num)
            else:
                print(".", end="")'''
    ...
    for iter in range(iter_num):
        if 'Demo_result{}.pkl'.format(iter) not in os.listdir():
            ...
            futures = [asyncio.ensure_future(property_query(sem, spo)) for spo in spo_set[iter*10000:min((iter+1)*10000,len(spo_set)-1)]]

            #task들을 비동기적으로 실행
            answer = await asyncio.gather(*futures)

            with open('Demo_result{}.pkl'.format(iter), 'wb') as f:
