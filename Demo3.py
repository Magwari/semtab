...
with open('sec_triple.pkl', 'rb') as f:
...
async def main():
    ...
    def result_return(spo):
        ...
        query3_2 = """SELECT distinct ?subject ?property 
                            ...
                            UNION {?subject ?property \""""+str(o)+"""\"^^xsd:dateTime.} 
                            FILTER (regex(str(?s) , \""""+str(s)+"""\")) 
                            SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. } 
                            }"""
    ...
    for iter in range(iter_num):
        if 'Demo2_result{}.pkl'.format(iter) not in os.listdir():
            ...
            with open('Demo2_result{}.pkl'.format(iter), 'wb') as f:
