...
with open('third_triple.pkl', 'rb') as f:
...
async def main():
    ...
    def result_return(spo):
        ...
        query1 = "\"".join(["""SELECT ?subject ?property ?object 
                            WHERE
                            {
                            {select ?subject where{?subject rdfs:label|schema:description|skos:altLabel """ ,str(s), """@en.} limit 2 offset 3}
        ...
        query3_1 = "\"".join(["""SELECT distinct ?subject ?property ?object 
                            ...
                            ?object rdfs:label|schema:description|skos:altLabel """ ,str(o), """@en. 
                            ?subject ?property ?object.
                            filter(regex(lcase(str(?s)),""", str(s), """))
                            SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. }
                            }
                            UNION
                            {
    ...
    for iter in range(iter_num):
        if 'Demo3_result{}.pkl'.format(iter) not in os.listdir():
            ...
            with open('Demo3_result{}.pkl'.format(iter), 'wb') as f:
